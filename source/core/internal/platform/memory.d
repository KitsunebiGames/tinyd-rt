module core.internal.platform.memory;
...
version (WebAssembly) {
	...
	*/
    ...
    void free(ubyte* ptr) @nogc @trusted nothrow {
        import c = core.stdc.stdlib : free;
        c.free(ptr);
    }

    ubyte[] malloc(size_t sz, string file = __FILE__, size_t line = __LINE__) @nogc @trusted nothrow {
        
        import c = core.stdc.stdlib : malloc;
        return (cast(ubyte*)c.malloc(sz))[0 .. sz];
    }

    ubyte[] realloc(ubyte* ptr, size_t newSize, string file = __FILE__, size_t line = __LINE__) @nogc @trusted nothrow {
        
        import c = core.stdc.stdlib : realloc;
        return (cast(ubyte*)c.realloc(ptr, newSize))[0 .. newSize];
    }

    ubyte[] reallocMain(ubyte* ptr, size_t newSize, string file = __FILE__, size_t line = __LINE__) @nogc @trusted nothrow {
    ...
    pragma(inline, true)
    ubyte[] pureRealloc(ubyte[] ptr, size_t newSize, string file = __FILE__, size_t line = __LINE__) pure @nogc @trusted nothrow {
        alias pRealloc = ubyte[]function(ubyte[], size_t, string file = __FILE__, size_t line = __LINE__) pure @nogc nothrow;
        auto pureRealloc = cast(pRealloc)&reallocMain;
        return pureRealloc(ptr, newSize, file, line);
    }

    ubyte[] calloc(size_t count, size_t size, string file = __FILE__, size_t line = __LINE__) @nogc @trusted nothrow {
        
        import c = core.stdc.stdlib : calloc;
        return (cast(ubyte*) c.calloc(count, size))[0 .. count * size];
    }
}