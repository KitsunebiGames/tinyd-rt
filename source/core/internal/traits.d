...
template ModifyTypePreservingTQ(alias Modifier, T) {
    static if (is(T U == immutable U))
        alias ModifyTypePreservingTQ = immutable Modifier!U;
    else static if (is(T U == shared inout const U))
        alias ModifyTypePreservingTQ = shared inout const Modifier!U;
    else static if (is(T U == shared inout U))
        alias ModifyTypePreservingTQ = shared inout Modifier!U;
    else static if (is(T U == shared const U))
        alias ModifyTypePreservingTQ = shared const Modifier!U;
    else static if (is(T U == shared U))
        alias ModifyTypePreservingTQ = shared Modifier!U;
    else static if (is(T U == inout const U))
        alias ModifyTypePreservingTQ = inout const Modifier!U;
    else static if (is(T U == inout U))
        alias ModifyTypePreservingTQ = inout Modifier!U;
...
private template substInoutForm(T) {
    static if (is(T == struct) || is(T == class) || is(T == union) || is(T == interface)) {
        ...
        alias substInoutForm = substInout!V[substInout!K];
    else static if (is(T : U[n], U, size_t n))
        alias substInoutForm = substInout!U[n];
    else static if (is(T : U[], U))
        alias substInoutForm = substInout!U[];
    else static if (is(T : U*, U))
        alias substInoutForm = substInout!U*;
    else
        alias substInoutForm = T;
}

/// used to declare an extern(D) function that is defined in a different module
template externDFunc(string fqn, T:
    FT*, FT) if (is(FT == function)) {
...
template isAssignable(Lhs, Rhs = Lhs) {
    enum isAssignable = __traits(compiles, lvalueOf!Lhs = rvalueOf!Rhs) && __traits(
            compiles, lvalueOf!Lhs = lvalueOf!Rhs);
}
...
template isInnerClass(T) if (is(T == class)) {
    static if (is(typeof(T.outer))) {
        ...
        enum isInnerClass = __traits(isSame, typeof(T.outer), __traits(parent, T)) && !hasOuterMember!(
...
template maxAlignment(Ts...) if (Ts.length > 0) {
    enum maxAlignment =
    {
        size_t result = 0;
        static foreach (T; Ts)
...
template hasElaborateCopyConstructor(S) {
    static if (__traits(isStaticArray, S)) {
        enum bool hasElaborateCopyConstructor = S.sizeof && hasElaborateCopyConstructor!(
                BaseElemOf!S);
    } else static if (is(S == struct)) {
...
template DynamicArrayTypeOf(T) {
     ...
     */
            ...
            alias staticMap = AliasSeq!(F!(T[0]), F!(T[1]), F!(T[2]), F!(T[3]), F!(T[4]), F!(T[5]), F!(
                    T[6]));
...
*/
    ...
    enum isTrue(Type, string member) = __traits(compiles, {
            static if (__traits(getMember, Type, member)) {
            } else
                static assert(0);
        });
