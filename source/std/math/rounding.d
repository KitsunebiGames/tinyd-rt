...
{
    return core.stdc.math.nearbyint(cast(double)x);
}
...
auto round(real x) @trusted nothrow @nogc
{
    ...
    {
        return core.stdc.math.round(cast(double)x);
    }
}
...
long lround(real x) @trusted nothrow @nogc
{
    version (CRuntime_DigitalMars)
        ...
        return core.stdc.math.llround(cast(double)x);
}
...
real trunc(real x) @trusted nothrow @nogc pure
{
    ...
    {
        return core.stdc.math.trunc(cast(double)x);
    }
}
